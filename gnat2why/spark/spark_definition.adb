...
package body SPARK_Definition is
   ...
   begin
      ...
      if Is_Subprogram (E) then
         ...
         --  Type_Invariant and Default_Initial_Condition. This does not
         --  include the functions generated for Predicate aspects, as these
         --  functions are translated to check absence of RTE in the predicate
         --  in the most general context.

         if Subprogram_Is_Ignored_For_Proof (E)

           --  Ignore generic subprograms

           or else Ekind (E) in Generic_Subprogram_Kind

           --  Ignore subprograms that front-end generates to analyze
           --  default expressions. They have no spec, only body and
           --  whose Is_Eliminated flag is set. Unlike user's subprograms
           --  with pragma Eliminated, they do come not from source. See
           --  Freeze.Process_Default_Expressions for details.

           or else (Is_Eliminated (E) and then not Comes_From_Source (E))
         then
            return;
         end if;
      end if;
   ...
   begin
      ...
      if Ekind (E) in E_Function and then Is_Predicate_Function (E) then
         declare
      ...
      if Ekind (E) in SPARK_Pragma_Scope_With_Type_Decl then
         declare
            Result : constant Node_Id := SPARK_Pragma (E);
         begin
      ...
      --  For some special cases, returning a SPARK_Mode would trip the rest of
      --  the analysis.
      ...
      if Ekind (E) in E_Loop_Parameter | E_Loop
        or else (Ekind (E) in E_Constant | E_Variable and then
                 Nkind (Parent (E)) = N_Iterator_Specification)
      then
         return
      ...
      --  For subprograms, to get the right entity node, we need to go one
      --  level up.
      ...
      --  package, we need to handle the special cases where the entity comes
      --  from the private declarations of the spec (first case) or the
      --  statements of the body (second case).
